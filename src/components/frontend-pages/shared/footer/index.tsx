...
import IconInstagram from 'src/assets/images/logos/zainzo.png';
import IconTiktok from 'src/assets/images/logos/zainzo.png';
import IconLinkedin from 'src/assets/images/logos/zainzo.png';
import IconYoutube from 'src/assets/images/logos/zainzo.png';

/* ---------------------------- Data menu footer ---------------------------- */
type FooterItem = { title: boolean; titleText: string; to?: string; link?: string };
...
const footerLinks: FooterGroup[] = [
  {
    ...
    children: [
      ...
      { title: false, titleText: 'Zainzo Book', to: '/book' },
      // { title: false, titleText: 'Zainzo Invoice', to: '/invoice' },
      { title: false, titleText: 'Zainzo Contact', to: '/contact' },
      // { title: false, titleText: 'Zainzo People', to: '/people' },
    ],
  },
  {
    id: 2,
  ...
  },
  {
    ...
    children: [
      ...
      { title: false, titleText: 'Kebijakan Privasi', link: 'https://help.zainzo.com/kebijakan-privasi/' },
...
function SmartLink({
  ...
  const baseSx = {
    ...
    textDecoration: 'none',
    '&:hover': { color: (t: any) => t.palette.primary.main },
    ...sx,
  };
  return isExternal ? (
...
function DesktopSection({ group }: { group: FooterGroup }) {
  ...
  return (
    <Box sx={{ minWidth: 180 }}>
      ...
      <Box>
        {items.map((it, idx) => (
          <SmartLink key={idx} to={it.to ?? it.link ?? '#'}>{it.titleText}</SmartLink>
        ))}
      </Box>
    </Box>
  );
  ...
  return (
    ...
    >
      ...
      <AccordionDetails sx={{ px: 0, pb: 1.25, pt: 0 }}>
        {items.map((it, idx) => (
          <SmartLink key={idx} to={it.to ?? it.link ?? '#'} sx={{ py: 1 }}>
            {it.titleText}
          </SmartLink>
        ))}
      </AccordionDetails>
        ...
        <Stack direction="row" alignItems="center" spacing={2}>
          ...
          <Tooltip title="Youtube">
            <MuiLink href={LINK_YOUTUBE} target="_blank" rel="noopener noreferrer" sx={{ display: 'flex', alignItems: 'center' }}>
              <img src={IconYoutube} alt="youtube" width={22} height={22} />
            </MuiLink>
          </Tooltip>
        </Stack>
